...
(() => {
  ...
  Promise.all([
    fetch(registryUrl,{cache:'no-store'}).then((response)=>{if(!response.ok) throw new Error('Freshness registry unavailable'); return response.json();}),
    ...
    const cards = records.map((record)=>`<article class="freshness-card" data-state="${escapeHtml(record.state || 'bounded')}"><div class="freshness-card-top"><span class="freshness-domain">${escapeHtml(record.label)}</span><span class="freshness-state">${escapeHtml(record.status)}</span></div><div class="freshness-date"><span>${escapeHtml(record.dateLabel)}</span><strong>${escapeHtml(formatDate(record.date))}</strong></div><p class="freshness-detail">${escapeHtml(record.detail)}</p><a class="freshness-link" href="${escapeHtml(safeHref(record.href))}">Open ${escapeHtml(record.label)} →</a></article>`).join('');
    mount.innerHTML = `<div class="freshness-head"><div><div class="freshness-kicker">Current Snapshot</div><h2 id="freshness-title">Evidence freshness by domain</h2></div><p class="freshness-note">${escapeHtml(registry.notice)}</p></div><div class="freshness-grid">${cards}</div>`;
  }).catch(()=>{ mount.innerHTML = '<div class="freshness-loading" role="status">Freshness metadata is temporarily unavailable. Existing academic content remains unchanged.</div>'; });
})();
