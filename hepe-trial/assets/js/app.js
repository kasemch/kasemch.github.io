...
function render(r){
 ...
 const submitted=previewStatus==='SUBMITTED';
 $('#review-note').textContent=submitted?'ส่งเข้าสู่ Review แล้ว — รอ Reviewer Decision จริง':(r?.preview?.preview_session_id?'Preview พร้อมส่ง Review':'ยังไม่มี Preview');
 const btn=$('#submit-review');if(btn){btn.disabled=submitted||!r?.preview?.preview_session_id;btn.textContent=submitted?'Submitted — Awaiting Reviewer':'Submit for Review';}
 if(submitted){$('#action').textContent='Human Decision Gate: ยังไม่มี reviewer decision และระบบจะไม่อนุมัติแทนผู้ใช้';}
...
async function submitReview(){
 say('กำลังส่ง Preview เข้าสู่ Review…');
 const before=await readiness();if(before?.preview?.preview_status==='SUBMITTED'){say('Preview ถูกส่งเข้าสู่ Review แล้ว','ok');return before;}
 const {data,error}=await client.rpc('hepe_submit_tqf3_preview_by_code',args());
 if(error){
   if(/INVALID_PREVIEW_STATE/i.test(error.message||'')){
     const after=await readiness();
     if(after?.preview?.preview_status==='SUBMITTED'){say('Preview ถูกส่งเข้าสู่ Review แล้ว','ok');return after;}
   }
   throw error;
 }
 $('#action').textContent='ส่ง Preview เข้าสู่ Review สำเร็จ';return data;
}
async function autoRun(){let r=await readiness();if(!r?.tqf3?.record_id){await createDraft();r=await readiness();}if(!r?.preview?.preview_session_id){await createPreview();r=await readiness();}if(r?.preview?.preview_session_id&&r?.preview?.preview_status==='READY_FOR_REVIEW'){await submitReview();r=await readiness();}if(r?.preview?.preview_status==='SUBMITTED')say('B03.17B สำเร็จ: รอ Reviewer Decision จริง','ok');else say('Authenticated workspace พร้อมใช้งาน','ok');}
async function boot(){say('กำลังตรวจ session…');cfg=await fetch('./config/state.json?v=11',{cache:'no-store'}).then(r=>r.json());$('#course').textContent=`${cfg.course_code} · ${cfg.programme_code} · AY ${cfg.academic_year}/${cfg.term_code}`;syncCooldown();const {data:{session},error}=await client.auth.getSession();if(error)throw error;$('#login').hidden=!!session;$('#workspace').hidden=!session;if(!session){say('ยังไม่พบ session — ระบบจะไม่ส่ง Magic Link ซ้ำจนกว่าคุณจะกดเอง','warn');return;}clearCooldown();const ctx=await client.rpc('hepe_my_context');if(ctx.error)throw ctx.error;$('#identity').textContent=`${ctx.data?.display_label??''} · ${(ctx.data?.roles??[]).map(r=>r.role_code).join(', ')}`;await autoRun();}
