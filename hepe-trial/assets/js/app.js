...
function syncCooldown(){
 ...
 btn.disabled=true;btn.textContent=`ส่งได้อีกครั้งใน ${fmtRemaining(remaining)}`;note.textContent='ระบบพักการส่งอีเมลไว้ชั่วคราวเพื่อป้องกัน rate limit และการกดซ้ำ';
...
async function boot(){say('กำลังตรวจ session…');cfg=await fetch('./config/state.json?v=10',{cache:'no-store'}).then(r=>r.json());$('#course').textContent=`${cfg.course_code} · ${cfg.programme_code} · AY ${cfg.academic_year}/${cfg.term_code}`;syncCooldown();
...
async function signIn(e){e.preventDefault();
 ...
 if(Date.now()<cooldownUntil()){syncCooldown();say('ยังอยู่ในช่วงพักการส่ง Magic Link กรุณารอให้ตัวนับหมดก่อน','warn');return;}
 ...
 if(error){const rateLimited=/rate limit/i.test(error.message||'');setCooldown(rateLimited?RATE_LIMIT_COOLDOWN_MS:NORMAL_COOLDOWN_MS);say(rateLimited?'Supabase จำกัดการส่งอีเมลชั่วคราว ระบบพักปุ่ม 15 นาทีเพื่อป้องกันการส่งซ้ำ':error.message,'warn');return;}
 setCooldown(NORMAL_COOLDOWN_MS);say('ส่ง Magic Link แล้ว ใช้อีเมลฉบับล่าสุดเพียงฉบับเดียว','ok');}
$('#login-form').addEventListener('submit',e=>signIn(e).catch(x=>{setCooldown(NORMAL_COOLDOWN_MS);say(x.message,'warn');}));$('#refresh').onclick=()=>readiness().catch(x=>say(x.message,'warn'));$('#submit-review').onclick=()=>submitReview().then(readiness).catch(x=>say(x.message,'warn'));$('#logout').onclick=async()=>{await client.auth.signOut();location.reload();};
