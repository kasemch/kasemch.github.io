...
let saveTimer=null,staticBound=false,aiUndoStack=[],aiSectionRuns={},aiSectionSuggestionCache={},reviewQueueIndex=0,reviewQueueFilter='ALL',reuseLineage=[];
...
function courseDescriptionKeywords(){
  const text=(curriculumCtx?.description?.description_th||'').replace(/[(),.]/g,' ').split(/\s+/).map(x=>x.trim()).filter(x=>x.length>=4);
  const stop=new Set(['และ','หรือ','การ','ของ','ที่','ใน','เพื่อ','โดย','จาก','กับ','เป็น','ผู้เรียน','นักศึกษา','รายวิชา']);
  return [...new Set(text.filter(x=>!stop.has(x)))].slice(0,8);
}
...
function fieldSuggestion(section,action,target,row=null){
  ...
  if(section==='tqf5'){
    ...
    if(target==='#improvement-plan'){
      ...
      return[mk('field-write-cqi','โครง CQI จากปัญหาที่ผู้ใช้กรอกจริง',value||scaffold,{severity:'SUGGESTION',why:'ข้อเสนอใช้เฉพาะข้อความปัญหาที่มีอยู่ใน Working TQF5 และเว้นส่วนการตัดสินใจให้ผู้ใช้'})];
    }
...
function aiQuickButtons(section,target,row=null,actions=['write','check','refine']){
  ...
  const visible=actions.includes('check')?'check':actions[0];
  const more=actions.filter(x=>x!==visible);
  ...
  if(!more.length)return '<span class="ai-field-actions compact">'+main+'</span>';
  ...
  return '<span class="ai-field-actions compact">'+main+menu+'</span>';
}
...
function renderReviewQueueV33(){
  ...
  const all=readinessGapList(),items=filteredReviewQueue();
  const blocking=all.filter(x=>x.state==='BLOCKING').length,warnings=all.filter(x=>x.state==='WARNING').length;
  if(!all.length){host.innerHTML='<div class="notice ok">Review Queue: ไม่พบ BLOCKING/WARNING ที่มี target</div>';return;}
  ...
  host.innerHTML='<div class="section-title"><div><h3>Review Queue Mode</h3><div class="help">Remaining '+all.length+' · BLOCKING '+blocking+' · WARNING '+warnings+' · แก้จากความเสี่ยงสูงก่อน</div></div>'+
    '<select id="rq-filter"><option value="ALL">ทั้งหมด</option><option value="BLOCKING">BLOCKING</option><option value="WARNING">WARNING</option><option value="TQF3">TQF3</option><option value="TQF5">TQF5</option><option value="VERIFICATION">Verification</option></select></div>'+
    (x?'<div class="review-queue-item"><div class="actions"><span class="badge '+(x.state==='BLOCKING'?'danger':'warn')+'">'+esc(x.state)+'</span><span class="badge info">'+esc(x.group)+'</span><span class="badge info">'+(reviewQueueIndex+1)+' / '+items.length+'</span></div><strong>'+esc(x.name)+'</strong><div>'+esc(x.detail)+'</div></div>'+
    '<div class="actions"><button type="button" class="btn small" id="rq-prev">ก่อนหน้า</button><button type="button" class="btn small primary" id="rq-jump">ไปแก้จุดนี้</button><button type="button" class="btn small good" id="rq-resolve-next">ตรวจซ้ำแล้วไปข้อถัดไป</button><button type="button" class="btn small" id="rq-next">ถัดไป</button></div>':'<div class="notice info">ไม่มีรายการตามตัวกรอง</div>');
  ...
  $('#rq-resolve-next').onclick=()=>{
    ...
    renderReadiness();
    const fresh=filteredReviewQueue();
    if(fresh.some(y=>y.target===target)){say('รายการนี้ยังไม่ผ่าน readiness — กรุณาแก้ข้อมูลก่อน','warn');jumpToReadinessTarget(target);return;}
    say('รายการเดิมผ่านแล้ว ไปข้อถัดไป','ok');
    reviewQueueIndex=Math.min(reviewQueueIndex,Math.max(fresh.length-1,0));
    renderReviewQueueV33();
    const n=fresh[reviewQueueIndex];if(n)jumpToReadinessTarget(n.target);
  };
}
...
function applyReuseDecisionV33(action,prev){
  ...
  reuseLineage.push({action:'FILL_BLANKS',source_version:prev.version_no,target_version:docCtx?.tqf3?.current_version_no,fields,at:new Date().toISOString()});
  aiDecisions.push({document:'TQF3',section:'REUSE_UPDATE',suggestion_id:'FILL_BLANKS_FROM_PRIOR',decision:'EDITED_AND_ACCEPTED',text:'Filled '+filled+' blank top-level fields only',source_basis:'Working Version '+prev.version_no,working_version:docCtx?.tqf3?.current_version_no,rationale:'User explicitly approved fill-blank-only carry-forward',decided_at:new Date().toISOString()});
...
async function boot(){
  cfg=await fetch('./config/state.json?v=34',{cache:'no-store'}).then(r=>r.json()).catch(()=>({}));
