...
function applySuggestionToField(i){
  ...
  say('นำข้อความที่ผู้ใช้แก้แล้วไปใส่ช่องเป้าหมายเรียบร้อย','ok');
}
...
async function saveTqf5(){
  ...
  const {data,error}=await client.rpc('hepe_create_tqf5_working_draft_by_code',{...courseArgs(),p_payload:collectTqf5(),p_source_reference:'HEPE Fast TQF Portal v29 / matrix-weekly-evidence-submission / NON-PRODUCTION'});if(error)throw error;
...
async function boot(){
  cfg=await fetch('./config/state.json?v=29',{cache:'no-store'}).then(r=>r.json()).catch(()=>({}));
