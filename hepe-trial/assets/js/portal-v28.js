...
let versionHistory=[],evidenceWorkspace=null,cqiContext=null,dashboardCtx=null;
let activeTab='tqf3',activeAiSection='curriculum',aiSuggestions=[],aiDecisions=[];
let saveTimer=null,staticBound=false;
...
async function loadDashboard(){
  ...
  const {data,error}=await client.rpc('hepe_fast_tqf_programme_dashboard',{
    p_programme_code:$('#programme-select').value,
    p_academic_year:$('#year-select').value,
    p_term_code:$('#term-select').value
  });
  if(error)throw error;
  dashboardCtx=data;renderDashboard();
}
...
function filterDashboardRows(rows=dashboardCtx?.courses||[]){
  ...
  const filtered=rows.filter(x=>!q || (x.course_code+' '+x.title_th+' '+(x.title_en||'')).toLowerCase().includes(q));
  $('#dashboard-body').innerHTML=filtered.map(x=>'<tr>'+
    ...
    '<td>'+(x.verification_updated_at?new Date(x.verification_updated_at).toLocaleDateString('th-TH'):'—')+'</td>'+
    '</tr>').join('') || '<tr><td colspan="7">ไม่พบรายวิชา</td></tr>';
}
...
async function saveTqf5(){
  ...
  const {data,error}=await client.rpc('hepe_create_tqf5_working_draft_by_code',{...courseArgs(),p_payload:collectTqf5(),p_source_reference:'HEPE Fast TQF Portal v27 / autosave-dashboard-evidence / NON-PRODUCTION'});if(error)throw error;
...
async function boot(){
  cfg=await fetch('./config/state.json?v=28',{cache:'no-store'}).then(r=>r.json()).catch(()=>({}));
